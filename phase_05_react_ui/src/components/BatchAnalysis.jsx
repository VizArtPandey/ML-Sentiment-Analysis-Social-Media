...
function exportCSV(results) {
  ...
  const rows = results.map((r, i) => {
    ...
    const cell = (k) =>
      r[k] ? `${r[k].label} (${Math.round(r[k].confidence * 100)}%)` : "N/A";
    return [
      i + 1,
      `"${r.text.replace(/"/g, '""')}"`,
      label,
...
function ModelCell({ data }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-xs font-semibold ${c.badge}`}
    >
    ...
    <>
      ...
      >
        ...
        <td className="px-4 py-3">
          <span
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs font-bold ${c.badge} capitalize`}
          >
            <span className={`w-1.5 h-1.5 rounded-full ${c.dot}`} />
            {label}
          </span>
              ...
              {models.map((m) => {
                ...
                if (!d)
                  return (
                    <div
                      key={m.key}
                      className="rounded-xl border border-slate-200 p-3 bg-white text-center"
                    >
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-xs font-bold ${mc.badge} capitalize mb-2`}
                    >
                      {d.label}
                    </span>
                    <div className="space-y-1">
                      {SENTIMENT_LABELS.map((lbl) => {
                        const score = d.scores?.[lbl] ?? 0;
                        return (
...
export default function BatchAnalysis() {
  ...
  const handleFile = useCallback((e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        setErrMsg("Could not parse CSV. Make sure it is UTF-8 encoded.");
        setStage("error");
      }
    };
  ...
  const runBatch = useCallback(async () => {
    ...
    if (!texts.length) {
      setErrMsg("No text found in the selected column.");
      setStage("error");
      return;
    }
    setStage("running");
    ...
    for (let i = 0; i < texts.length; i += CHUNK) {
      ...
      try {
        ...
        setErrMsg(
          "API is offline or unreachable. Start backend on port 8000 and rerun batch analysis.",
        );
        setStage("error");
        return;
      }
    ...
    return (
      <div className="space-y-6">
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-slate-400 text-sm mt-1">
              or click to browse — UTF-8 encoded, with a header row
            </p>
          </div>
        ...
        <div className="bg-slate-50 rounded-2xl border border-slate-200 p-5 text-sm text-slate-600 space-y-1">
          ...
          <p>
            3. Click <strong>Run Batch Analysis</strong> — all 5 models score
            every row.
          </p>
          <p>
            4. Expand any row for per-model scores, then export the full table
            as CSV.
          </p>
        </div>
      </div>
          ...
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between flex-wrap gap-3">
            <div>
              <p className="font-bold text-slate-800">
                CSV loaded — {rows.length} rows detected
              </p>
              ...
              <thead className="bg-slate-50 sticky top-0">
                <tr>
                  {headers.map((h, i) => (
                    <th
                      key={i}
                      className={`px-4 py-2 text-left text-xs font-bold text-slate-500 ${i === textCol ? "bg-violet-50 text-violet-700" : ""}`}
                    >
                      {h || `Col ${i + 1}`} {i === textCol && "✓"}
                    </th>
                  ))}
                ...
                {rows.slice(0, 5).map((row, ri) => (
                  <tr key={ri} className="border-t border-slate-50">
                    {row.map((cell, ci) => (
                      <td
                        key={ci}
                        className={`px-4 py-2 text-slate-600 max-w-xs truncate ${ci === textCol ? "bg-violet-50/50" : ""}`}
                      >
                        {cell}
                      </td>
                    ))}
  ...
  if (stage === "running")
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-6">
        <div className="w-20 h-20 rounded-2xl bg-violet-100 flex items-center justify-center animate-pulse text-3xl">
          🔍
        </div>
        <div className="text-center">
          <p className="font-bold text-slate-700 text-lg">
            Analysing {rows.length} rows…
          </p>
          <p className="text-sm text-slate-400 mt-1">
            Running all 5 models — please wait
          </p>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          ...
          <div
            key={label}
            className={`rounded-2xl border p-4 text-center ${bg} ${border}`}
          >
            ...
            <div className="h-1.5 bg-white/70 rounded-full mt-2 overflow-hidden">
              <div
                ...
                style={{
                  width: `${total ? ((sentCounts[label] || 0) / total) * 100 : 0}%`,
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between flex-wrap gap-3">
          <div>
            <p className="font-bold text-slate-800">
              {results.length} rows analysed
            </p>
            <p className="text-xs text-slate-400 mt-0.5">
              Click any row to expand per-model scores
            </p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-400 w-10">
                  #
                </th>
                ...
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 w-32">
                  Confidence
                </th>
                {["VADER", "LR", "RF", "SVM", "BiLSTM"].map((m) => (
                  <th
                    key={m}
                    className="px-3 py-3 text-center text-xs font-bold text-slate-500"
                  >
                    {m}
                  </th>
                ))}
