...
const MODEL_META = {
  'Calibrated Output':  { icon: '🎯', color: 'text-slate-700',  bg: 'bg-slate-100'  },
  VADER:                { icon: '📏', color: 'text-red-600',    bg: 'bg-red-50'    },
  'Logistic Regression':{ icon: '📈', color: 'text-blue-600',   bg: 'bg-blue-50'   },
...
export default function ResultCard({ modelName, label, confidence, scores }) {
  ...
  const m = MODEL_META[modelName] ?? { icon: '🤖', color: 'text-gray-600', bg: 'bg-gray-50' }
  ...
  return (
    <div className={`flex-1 min-w-0 rounded-2xl border p-4 flex flex-col gap-3 animate-slide-up ${s.bg} ${s.border} ${s.ring}`}>
      ...
      {/* Sentiment label */}
      <div>
        <div className={`text-2xl font-extrabold capitalize ${s.label}`}>{label}</div>
        <div className="mt-2">
