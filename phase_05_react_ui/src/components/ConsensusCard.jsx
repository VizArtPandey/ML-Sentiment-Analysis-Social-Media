...
export default function ConsensusCard({ results }) {
  ...
  const votes = {}
  const models = MODEL_ORDER.filter((k) => results[k])
  models.forEach((k) => {
  ...
  const agreement = Math.round((count / models.length) * 100)
  const avgConf   = models.reduce((s, k) => s + (results[k]?.confidence ?? 0), 0) / models.length

  return (
    <div className={`rounded-2xl border-2 p-5 animate-fade-in ${cfg.bg} ${cfg.border}`}>
      <div className="flex flex-col sm:flex-row sm:items-center gap-5">
        ...
        <div className="shrink-0 text-center sm:text-left">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            ...
            <span className="text-xs text-slate-500">{count} of {models.length} models agree</span>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-3">
          <div>
            <div className="flex justify-between text-xs mb-1.5">
              <span className="font-medium text-slate-600">Model agreement</span>
      ...
      <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-white/60">
        {MODEL_ORDER.filter((k) => results[k]).map((k) => {
