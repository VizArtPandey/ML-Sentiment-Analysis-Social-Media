...
const BILSTM_FIX_CMD =
  "pip install -r requirements.txt && python -m phase_04_rnn_bilstm.02_train_rnn";
const MODEL_NEEDS_TRAINING = {
  bilstm:
    "Not loaded locally — install TensorFlow or retrain, then restart the backend",
};
...
function extractApiError(error) {
  ...
  if (Array.isArray(detail))
    return detail
      .map((item) => item.msg || item.detail || "Invalid input")
      .join(" ");
...
function ModelPill({ modelKey, result }) {
    ...
    return (
      <div className="min-h-[112px] rounded-2xl border border-dashed border-slate-300 bg-slate-50/60 p-4 flex flex-col justify-between">
        <div className="flex items-center justify-between">
          ...
          <span className="w-8 h-8 rounded-xl bg-slate-200 text-slate-400 text-xs font-black grid place-items-center">
            {model.icon}
          </span>
        </div>
        <div>
  ...
  return (
    <div
      className={`min-h-[112px] rounded-2xl border-2 ${sentiment.border} ${sentiment.bg} p-4 flex flex-col justify-between shadow-sm`}
    >
      <div className="flex items-center justify-between gap-2">
        <span className={`text-xs font-black tracking-wide ${model.text}`}>
          {MODEL_LABEL[modelKey]}
        </span>
        <span
          className={`w-9 h-9 rounded-xl ${model.dot} text-white text-sm font-black grid place-items-center leading-none shadow-sm`}
        >
          {model.icon}
        </span>
      </div>
      <div>
        <div
          className={`text-2xl font-black capitalize leading-none ${sentiment.text}`}
        >
          {result.label}
        </div>
        <div className="flex items-end justify-between gap-3 mt-3">
          <div className="h-2.5 flex-1 rounded-full bg-white/80 border border-white overflow-hidden">
            <div
              className={`h-full rounded-full ${sentiment.dot}`}
              style={{ width: `${percent}%` }}
            />
          </div>
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-[160px_1fr_auto] gap-5 items-start">
        <div
          className={`rounded-3xl ${sentiment.panel} text-white p-5 min-h-[132px] flex flex-col justify-between shadow-lg`}
        >
          <div className="flex items-center justify-between">
          ...
          <div>
            ...
            <p className="text-sm font-semibold opacity-85 mt-2">
              {con.count}/{con.total || MODELS.length} agree
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <p
            className={`text-xl sm:text-2xl font-bold text-slate-900 leading-snug ${open ? "" : "line-clamp-3"}`}
          >
            {tweet.text}
          </p>
          <div className="flex items-center gap-2.5 mt-4 flex-wrap">
            ...
            {tweet.hashtag && (
              <span className="text-sm px-3 py-1 rounded-full bg-cyan-50 text-cyan-700 border border-cyan-200 font-black">
                {tweet.hashtag}
              </span>
            )}
        ...
        <div className="shrink-0 flex lg:flex-col items-end gap-3">
          <div className="flex gap-1.5 items-center">
            {MODELS.map((key) => {
              ...
              const style = result
                ? (SENT_STYLE[result.label] ?? SENT_STYLE.neutral)
                : { dot: "bg-slate-200" };
              return (
                <span
                  key={key}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {MODELS.map((key) => (
              <ModelPill
                key={key}
                modelKey={key}
                result={tweet.predictions?.[key]}
              />
            ))}
          </div>
          {tweet.predictions?.bilstm && (
            <div className="rounded-3xl bg-slate-50 border border-slate-200 p-5">
              <p className="text-xs text-slate-500 font-black uppercase mb-3">
                BiLSTM score breakdown
              </p>
                ...
                {["positive", "negative", "neutral"].map((label) => {
                  ...
                  return (
                    <div key={label} className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-slate-700 font-black w-12 text-right tabular-nums">
                        {(value * 100).toFixed(0)}%
                      </span>
                    </div>
                  );
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {["positive", "negative", "neutral", "mixed"].map((key) => {
        ...
        return (
          <div
            key={key}
            className={`rounded-3xl border-2 p-5 text-center ${style.bg} ${style.border} shadow-sm`}
          >
            <div className={`text-5xl font-black ${style.text}`}>
              {counts[key]}
            </div>
            <div className="text-sm font-black text-slate-500 capitalize mt-1">
              {key}
            </div>
            <div className="h-2.5 bg-slate-200 rounded-full mt-4 overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.max(pct, counts[key] > 0 ? 4 : 0)}%`,
...
function TwitterConnectPanel({ token, onSave }) {
  ...
  return (
    <div
      className={`rounded-2xl border-2 transition-all ${saved ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}
    >
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p
              className={`text-sm font-bold ${saved ? "text-emerald-800" : "text-amber-800"}`}
            >
              {saved
                ? "Twitter/X Connected"
                : "Connect Twitter/X (for live tweets)"}
            </p>
            <p
              className={`text-xs ${saved ? "text-emerald-600" : "text-amber-600"}`}
            >
              {saved
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-bold ${saved ? "text-emerald-600" : "text-amber-600"}`}
          >
            {open ? "▲" : "▼"}
          </span>
        </div>
        ...
        <div className="px-4 pb-4 space-y-4 border-t border-white/60 pt-4">
          ...
          <div className="rounded-xl bg-white/80 border border-amber-200 p-4 space-y-2">
            ...
            <ol className="text-xs text-slate-600 space-y-1.5 list-decimal list-inside">
              ...
              <li>
                Copy <span className="font-bold">Bearer Token</span> (not API
                Key)
              </li>
              <li>Paste it below and click Save</li>
            </ol>
            <p className="text-xs text-slate-400 mt-2">
              Token is stored in your browser only — never sent anywhere except
              your local backend.
            </p>
          </div>
          ...
          <div className="rounded-xl bg-slate-900 text-slate-300 p-4 text-xs space-y-2">
            ...
            <p className="text-slate-400">
              A saved model exists, but the backend also needs TensorFlow
              installed in the active Python environment. Retrain only if the
            ...
            <p className="text-slate-400">
              Then restart the backend. Training saves{" "}
              <span className="font-mono text-amber-300">
                bilstm_best.keras
              </span>{" "}
              for TensorFlow/Keras 3 and a local{" "}
              <span className="font-mono text-amber-300">.h5</span> copy for
              older tooling.
            </p>
          </div>
        </div>
...
export default function LiveEval() {
  ...
  const fetchTweets = useCallback(
    async (tagValue, token) => {
      const tag = normalizeHashtag(tagValue);
      if (!tag) {
        setError("Enter a hashtag first.");
        setTweets([]);
        return;
      }
      setLoading(true);
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-6">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-50 border border-cyan-200 text-cyan-700 text-xs font-semibold">
          <span
            className={`w-1.5 h-1.5 rounded-full ${isLive ? "bg-emerald-500" : "bg-amber-400"} animate-pulse`}
          />
          {isLive
            ? "Live X/Twitter Feed"
        ...
        <p className="text-slate-500 text-base max-w-2xl mx-auto">
          Enter a keyword or topic to fetch the newest 10 matching posts, sort
          by timestamp, and run the full sentiment model suite in real time.
        </p>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <label className="flex-1">
            <span className="block text-xs font-bold text-slate-500 uppercase mb-1.5">
              Topic or Keyword to evaluate
            </span>
            <div className="flex items-center rounded-xl border border-slate-300 bg-white focus-within:border-indigo-400 focus-within:ring-4 focus-within:ring-indigo-100 overflow-hidden">
              <input
                value={hashtag}
                onChange={(e) => setHashtag(e.target.value)}
                placeholder="OpenAI, MachineLearning, ClimateChange…"
                className="w-full bg-transparent px-4 py-3 text-sm text-gray-900 outline-none placeholder:text-slate-300"
                disabled={loading}
              />
            </div>
          </label>
          <div className="flex items-end gap-2">
            ...
            <button
              ...
              disabled={loading || (!activeTag && !hashtag)}
      ...
      {!loading && !tweets.length && !error && (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          <div className="w-16 h-16 rounded-2xl bg-slate-900 text-white grid place-items-center font-bold items-center shadow-xl">
            <span className="text-2xl">⚡</span>
          </div>
          <p className="text-slate-500 text-sm max-w-sm leading-relaxed">
            {activeTag
              ? `No posts returned for "${activeTag}". Try another keyword or refresh in a minute.`
              : "Enter a keyword above and click evaluate to view sentiment across up to 10 recent posts."}
          </p>
        </div>
      )}
      ...
      {!loading && tweets.length > 0 && (
        <>
          ...
          <p className="text-center text-xs text-slate-400 pb-4">
            Click any post to expand model details; live X posts also show
            engagement metrics.
          </p>
        </>
      )}
