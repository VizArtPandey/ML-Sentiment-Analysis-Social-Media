...
function ModelPill({ modelKey, result }) {
  ...
  if (!result) {
    return (
      <div className="min-h-[112px] rounded-2xl border border-slate-200 bg-slate-50 p-4 flex flex-col justify-between opacity-70">
        <div className="flex items-center justify-between">
          <span className="text-xs font-black tracking-wide text-slate-400">{MODEL_LABEL[modelKey]}</span>
          <span className="w-8 h-8 rounded-xl bg-slate-200 text-slate-500 text-xs font-black grid place-items-center">{model.icon}</span>
        </div>
        <div>
          <p className="text-xl font-black text-slate-400">Offline</p>
  ...
  return (
    <div
      ...
      title={`${MODEL_LABEL[modelKey]}: ${result.label} (${percent}%)`}
    >
      <div className="flex items-center justify-between gap-2">
        <span className={`text-xs font-black tracking-wide ${model.text}`}>{MODEL_LABEL[modelKey]}</span>
        <span className={`w-9 h-9 rounded-xl ${model.dot} text-white text-sm font-black grid place-items-center leading-none shadow-sm`}>
          {model.icon}
        </span>
      </div>
      <div>
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-[160px_1fr_auto] gap-5 items-start">
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-xl sm:text-2xl font-bold text-slate-900 leading-snug ${open ? '' : 'line-clamp-3'}`}>
            {tweet.text}
          </p>
          <div className="flex items-center gap-2.5 mt-4 flex-wrap">
            ...
            <span className={`text-sm px-3 py-1 rounded-full border font-black ${sentiment.badge}`}>
      ...
      {open && (
        <div className="mt-6 pt-6 border-t border-slate-200 space-y-6">
          ...
          <MetricsRow metrics={tweet.metrics} />

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
            {MODELS.map(key => (
              <ModelPill key={key} modelKey={key} result={tweet.predictions?.[key]} />
            ))}
          </div>
          ...
          {tweet.predictions?.bilstm && (
            <div className="rounded-3xl bg-slate-50 border border-slate-200 p-5">
              ...
              <div className="space-y-3">
                {['positive', 'negative', 'neutral', 'mixed'].map(label => {
...
function DistBar({ tweets }) {
  ...
  const items = ['positive', 'negative', 'neutral', 'mixed']
  return (
      ...
      {items.map(key => {
        ...
        return (
          <div key={key} className={`rounded-3xl border-2 p-5 text-center ${style.bg} ${style.border} shadow-sm`}>
            ...
            <div className="h-2.5 bg-white/80 rounded-full mt-4 overflow-hidden border border-white">
              <div
                className={`h-full rounded-full ${style.dot}`}
                style={{ width: `${total ? (counts[key] / total) * 100 : 0}%`, transition: 'width 0.8s ease-out' }}
              />
            </div>
          </div>
        )
...
export default function LiveEval() {
  ...
  const fetchTweets = useCallback(async tagValue => {
    ...
    setError(null)
    try {
      const data = await getLiveEval(10, tag)
      setTweets(data)
      setActiveTag(tag)
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-8">
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-50 border border-cyan-200 text-cyan-700 text-xs font-semibold">
          ...
          Live Twitter Hashtag Evaluation
        </div>
        <h1 className="text-4xl font-extrabold text-gray-900">
          Live Tweet <span className="text-gradient">Eval</span>
        </h1>
        <p className="text-slate-500 text-base max-w-2xl mx-auto">
          Enter a hashtag to pull the newest 10 matching X/Twitter posts, sort them by timestamp, and run the sentiment model suite.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card p-4 sm:p-5">
        <div className="flex flex-col md:flex-row gap-3">
          <label className="flex-1">
            <span className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Hashtag</span>
            <div className="flex items-center rounded-xl border border-slate-300 bg-white focus-within:border-indigo-400 focus-within:ring-4 focus-within:ring-indigo-100">
              <span className="pl-4 pr-1 text-slate-400 font-bold">#</span>
              <input
                value={hashtag}
          ...
          <div className="flex items-end gap-2">
            <button type="submit" disabled={loading} className="btn-primary h-12 px-5 flex items-center gap-2">
              {loading && <span className="w-3.5 h-3.5 border border-white/40 border-t-white rounded-full animate-spin" />}
              Evaluate
            </button>
            <button type="button" onClick={handleRefresh} disabled={loading || (!activeTag && !hashtag)} className="btn-secondary h-12 px-4">
              Refresh
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap mt-4">
          ...
          {activeTag && (
            <span className="text-xs text-slate-400">
              Auto-refreshes every 60 seconds after evaluation
            </span>
          )}
        </div>
      </form>
      ...
      {!loading && !tweets.length && !error && (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          <div className="w-14 h-14 rounded-2xl bg-slate-900 text-white grid place-items-center font-bold text-xl">#</div>
          <p className="text-slate-500 text-sm max-w-sm">
            {activeTag
              ? `No public English posts were returned for #${activeTag}. Try another hashtag or refresh in a minute.`
              : 'Enter a hashtag to fetch the newest public posts from X/Twitter and evaluate the sentiment model consensus.'}
          </p>
        </div>
      )}
        ...
        <>
          <div className="rounded-3xl border-2 border-indigo-100 bg-white p-6 sm:p-7 shadow-xl shadow-indigo-100/50 space-y-5">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                <h2 className="text-2xl font-black text-gray-900">Hashtag Batch Summary</h2>
                <p className="text-sm text-slate-500 mt-1">
                  {tweets.length} newest posts · sorted by timestamp · model suite
                </p>
              </div>
              <span className="text-sm px-4 py-2 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 font-black">
                Newest first
              </span>
            </div>
          ...
          <p className="text-center text-xs text-slate-400 pb-4">
            Click any post to expand model details and public engagement metrics
          </p>
        </>
      )}
    </div>
